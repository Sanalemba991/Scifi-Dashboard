      ...
      {[...Array(50)].map((_, i) => (
        <div
          ...
          style={{
            ...
            background: i % 3 === 0 ? '#06b6d4' : i % 3 === 1 ? '#22d3ee' : '#0ea5e9',
          }}
        />
      ))}
    </div>
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden opacity-20">
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className="absolute text-9xl text-cyan-500/10 font-mono float"
          style={{
            left: `${15 + i * 15}%`,
            top: `${10 + (i % 3) * 30}%`,
  ...
  return (
    <div className="scanlines min-h-screen bg-[#050508] grid-bg hex-pattern">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 w-px h-full bg-gradient-to-b from-transparent via-blue-500/20 to-transparent animate-pulse" style={{ animationDelay: '2s' }} />
        ...
        <div className="absolute top-4 left-4 w-20 h-20 border-l-2 border-t-2 border-cyan-500/50 animate-pulse" />
        ...
        <div className="absolute bottom-4 right-4 w-20 h-20 border-r-2 border-b-2 border-cyan-500/50 animate-pulse" style={{ animationDelay: '1.5s' }} />
      </div>

      {/* Sidebar Toggle Button */}
      <button
        onClick={() => setIsSidebarOpen(!isSidebarOpen)}
        className="fixed top-6 left-6 z-50 glass glow-border p-3 rounded-lg text-cyan-500 hover-glow transition-all duration-300"
      ...
      {/* Fixed Sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-72 glass border-r border-cyan-500/30 flex flex-col z-40 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Logo */}
        <div className="p-6 border-b border-cyan-500/30">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 glass glow-border flex items-center justify-center text-cyan-500 text-2xl pulse-ring float">
              ⬡
            </div>
            <div>
              <h1 className={`text-cyan-500 font-bold tracking-widest text-lg neon-text-cyan ${glitchText ? 'glitch' : ''}`} data-text="NEXUS">
                NEXUS
              </h1>
            ...
            {systemStatus.map((sys, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`${sys.value === 100 ? "text-green-400" : "text-cyan-400"
                      }`}
                  >
                    {sys.value}%
                  </span>
      ...
      <main className={`flex-1 p-6 overflow-auto transition-all duration-300 ${isSidebarOpen ? 'ml-72' : 'ml-0'}`}>
        ...
        <header className="glass glow-border rounded-lg p-4 mb-6 flex items-center justify-between fade-in-down corner-accent">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-cyan-500 animate-pulse-glow">●</span>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="glass px-4 py-2 rounded text-xs text-cyan-400 animate-border-pulse border border-cyan-500/50 hover-glow">
              ◈ SECURE CONNECTION
            </div>
          </div>
        </header>
        ...
        <div className="mt-6 fade-in-up" style={{ animationDelay: '0.3s' }}>
          <div className="glass rounded-lg overflow-hidden border border-cyan-500/20">
            <div className="flex items-center gap-4 px-4 py-2 bg-cyan-900/20 border-b border-cyan-500/20">
              <span className="text-cyan-500 animate-pulse-glow text-xs">● REC</span>
              ...
              <div className="flex gap-2 data-stream whitespace-nowrap">
                {["█7A3F", "░B2E9", "█4C8D", "░1A5E", "█9F2B", "░C4E7", "█8D3A", "░2E6F", "█5B9C", "░A1D8",
                  ...
                  "█7A3F", "░B2E9", "█4C8D", "░1A5E", "█9F2B", "░C4E7", "█8D3A", "░2E6F", "█5B9C", "░A1D8"].map((code, i) => (
                    <span key={i} className={`text-xs font-mono ${i % 2 === 0 ? 'text-cyan-500/60' : 'text-blue-500/60'}`}>
                      {code}
                    </span>
                  ))}
              </div>
